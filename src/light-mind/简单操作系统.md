...
- [RISCV psABI文档](https://github.com/riscv-non-isa/riscv-elf-psabi-doc/releases/tag/v1.0)
# 正文
## 环境配置
### 官方工具链
安装所需的包：
...
- 浮点寄存器（`f0-f31`）在内核中不使用，因此这里不需要保存它们。通常，它们在线程切换期间保存和恢复。
...
> 最后需要告诉 CPU 异常处理程序的位置。这是通过在 `kernel_main` 函数中设置 `stvec` 寄存器来完成的：
```c
// kernel.c
void kernel_main(void) {
...
（`stvec`寄存器为异常处理程序入口，这里设置为上面自定义的`kernel_entry`函数的入口地址）
...
:::
![](assets/Pasted%20image%2020251001202119.png)
执行起来测试一下，可以看到`unimp`指令后面的logging测试语句无法被执行
```log
PANIC: kernel.c:66: unexpected trap scause=00000002, stval=00000000, sepc=8020014a
```
> 当 `scause` 的值为 2 时，表示“非法指令”，意味着程序试图执行无效指令。这正是 `unimp` 指令的预期行为！
![](assets/Pasted%20image%2020251001203202.png)
> 顺便检查一下 `sepc` 的值指向哪里。如果它指向调用 `unimp` 指令的行，那么一切都如预期：
```sh
apt install llvm-addr2line && llvm-addr2line -e kernel.elf 8020014a
/develop/MyOS-1000lines/kernel.c:168
```
![](assets/Pasted%20image%2020251001203048.png)

## 内存分配
![](assets/Pasted%20image%2020251001225828.png)
```
	. = ALIGN(4096);
    __free_ram = .;
    . += 64 * 1024 * 1024; /* 64MB */
    __free_ram_end = .;
```
> 在实现内存分配器之前，我们需要定义要由分配器管理的内存区域（如上）
...
:::
### 试试内存分配
![](assets/Pasted%20image%2020251014211807.png)
```c
[90 ms] [DEBUG] |kernel.c:177| paddr0=80221000, paddr1=80222000
```
相差0x1000，或4KB字节，正好是一页内存的大小

## 进程
> 进程是应用程序的一个实例。每个进程都有其独立的执行上下文和资源，如虚拟地址空间。

:::note
实际的操作系统将执行上下文作为一个称为**线程**的独立概念提供。这里为简单起见，将每个进程都视为只有一个线程
:::
### 进程控制块
如下定义一个*Process Control Block*
...
```
> `switch_context`将被调用者保存的寄存器保存到栈上，切换栈指针，然后从栈中恢复被调用者保存的寄存器。换句话说，执行上下文作为临时局部变量存储在栈上。
...
有了进程上下文切换之后，我们还需要一个创建进程的函数，它以入口点为参数，并返回指向创建的`process`结构的指针：
```c
...
![](assets/Pasted%20image%2020251015205751.png)
...
```
> 这里引入了两个全局变量。`current_proc`指向当前运行的进程。`idle_proc`指向空闲进程，即“当没有可运行进程时要运行的进程”。`idle_proc`在启动时创建为进程 ID 为`0`的进程
![](assets/Pasted%20image%2020251015212940.png)
...
![](assets/Pasted%20image%2020251018214207.png)
> 现在`sp`指向当前运行进程的*内核(不是用户)* 栈。此外，`sscratch`现在保存异常发生时原始的`sp`值（用户栈）。
